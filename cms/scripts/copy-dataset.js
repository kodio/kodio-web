...
; (async () => {
    ...
    const shouldCopy = await ask(`Copy Sanity dataset from ${exportFrom} to ${importTo}? (Y/n) `, 'y')
    if (['y', 'Y', 'yes', 'YES'].includes(shouldCopy)) {
        spawnSync('npm', ['run', 'sanity', '--', 'login'], options)
        spawnSync('npm', ['run', 'sanity', '--', 'dataset', 'export', exportFrom, tmpFileName], options)
        spawnSync('npm', ['run', 'sanity', '--', 'dataset', 'import', tmpFileName, importTo, '--replace'], options)
